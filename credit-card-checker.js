...
const luhnAlgorithm = (array) => {
    ...
    let sumArray = newArray.reduce((previousVal, currentVal) => previousVal + currentVal, 0);
    if (sumArray % 10 === 0) {
        return 'valid';
    } else {
        return 'invalid';
...
const filteredCompanies = (array) => {
    ...
    for (element of array) {
        if (uniqueVals.includes(element) === false) {
...
const idInvalidCardCompanies = (nestedArray) => {
    ...
    let uniqueCompanies = filteredCompanies(companies);
